...
#define STATIC_POOL_DEFINE(name, size, type, lock, unlock)                                                             \
    ...
    static static_pool_cb pool_cb_##name = {size, sizeof(type), pool_state_##name, pool_##name, lock, unlock}
 ...
 *
 */
#define STATIC_POOL_SET_LOCK(name, func)   pool_cb_##name.Lock = func

/**
 * @brief   Macro for Unlock function registration.
 *
 */
#define STATIC_POOL_SET_UNLOCK(name, func) pool_cb_##name.Unlock = func
